...
# Helper function to compute clean percentage wording without raw total subtractions
def calc_change(curr_val, prev_val, is_pct=False, is_cumulative=False):
    try:
        ...
        if p == 0:
            return "No change" if diff == 0 else "N/A"

        pct_change = (diff / p) * 100

        # For cumulative totals like Followers, show absolute count + percentage
        if is_cumulative:
            sign = "+" if diff > 0 else ""
            return f"{sign}{int(diff):,} ({pct_change:+.1f}%)"

        # For non-cumulative snapshot metrics (Reach, Views, Profile Views)
        if pct_change == 0:
            return "No change"
        elif pct_change > 0:
            return f"{pct_change:.1f}% increase"
        else:
            return f"{abs(pct_change):.1f}% decrease"
            
    except:
        return "N/A"

# 3. Build HTML Table directly in Python
...
<table style="width: 100%; border-collapse: collapse; margin: 15px 0; font-size: 14px;">
  ...
  <tbody>
    <tr>
      ...
      <td style="padding: 8px; border: 1px solid #e2e8f0;">{calc_change(curr.get('followers',0), prev.get('followers',0), is_cumulative=True)}</td>
    </tr>
...
# 4. Prompt OpenAI ONLY for executive summary and action items
prompt = f"""
...
print("Updated report sent successfully!")
